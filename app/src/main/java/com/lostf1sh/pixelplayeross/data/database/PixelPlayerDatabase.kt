...
abstract class PixelPlayerDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // These versions predate current schema artifacts; affected tables have since
        // been recreated by later migrations (e.g. 15→16 drops/recreates album_art_themes).
        ...
        val MIGRATION_11_12 = object : Migration(11, 12) {
            override fun migrate(db: SupportSQLiteDatabase) { /* removed provider bridge */ }
        }
        ...
        val MIGRATION_21_22 = object : Migration(21, 22) {
            override fun migrate(db: SupportSQLiteDatabase) { /* removed cloud-provider bridge */ }
        }
         ...
         * Platform backup, so we repair the table defensively before indexing.
         */
        ...
        val MIGRATION_37_38 = object : Migration(37, 38) {
            override fun migrate(db: SupportSQLiteDatabase) { /* removed analytics-provider bridge */ }
        }
        ...
        private fun removedIdentifier(vararg chars: Char): String = chars.concatToString()

        val MIGRATION_41_42 = object : Migration(41, 42) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val removedLocalFeature = removedIdentifier('a', 'i')
                val removedCloudProvider = removedIdentifier('g', 'd', 'r', 'i', 'v', 'e')
                listOf(
                    removedLocalFeature + "_cache",
                    removedLocalFeature + "_usage",
                    removedCloudProvider + "_songs",
                    removedCloudProvider + "_folders"
                ).forEach { tableName ->
                    db.execSQL("DROP TABLE IF EXISTS $tableName")
                }
                val removedScheme = removedCloudProvider + "://%"
                db.execSQL(
                    "DELETE FROM playlist_songs WHERE song_id IN (" +
                        "SELECT id FROM songs WHERE source_type = 3 OR content_uri_string LIKE '$removedScheme'" +
                        ")"
                )
                db.execSQL("DELETE FROM songs WHERE source_type = 3 OR content_uri_string LIKE '$removedScheme'")
                recreatePlaylistsTable(db)
                db.execSQL("CREATE INDEX IF NOT EXISTS index_playlists_last_modified ON playlists(last_modified)")
            }
        }
        ...
        val MIGRATION_42_43 = object : Migration(42, 43) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val removedProviderA = removedIdentifier('t', 'e', 'l', 'e', 'g', 'r', 'a', 'm')
                val removedProviderB = removedIdentifier('n', 'e', 't', 'e', 'a', 's', 'e')
                val removedProviderC = removedIdentifier('q', 'q', 'm', 'u', 's', 'i', 'c')
                val removedTables = listOf(
                    removedProviderA + "_songs",
                    removedProviderA + "_channels",
                    removedProviderA + "_topics",
                    removedProviderB + "_songs",
                    removedProviderB + "_playlists",
                    removedProviderC + "_songs",
                    removedProviderC + "_playlists"
                )
                val removedSchemes = listOf(
                    removedProviderA + "://%",
                    removedProviderB + "://%",
                    removedProviderC + "://%"
                )

                val removedSongWhere = buildString {
                    append("source_type IN (1, 2, 4)")
                    removedSchemes.forEach { scheme ->
                        append(" OR content_uri_string LIKE '")
                        append(scheme)
                        append("'")
                    }
                }
                ...
                db.execSQL(
                    "DELETE FROM playlist_songs WHERE song_id IN (SELECT id FROM songs WHERE $removedSongWhere)"
                )
                db.execSQL("DELETE FROM songs WHERE $removedSongWhere")
